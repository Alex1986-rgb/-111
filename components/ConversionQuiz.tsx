        ...
        <div className="bg-slate-50 rounded-[3.5rem] p-6 md:p-16 border border-slate-100 shadow-sm relative overflow-hidden">
          {!showResult ? (
                ...
                {steps[currentStep].options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-4 p-6 bg-white border-2 border-transparent hover:border-indigo-600 rounded-[2rem] text-left transition-all hover:shadow-xl hover:-translate-y-1 group"
                    >
                      ...
                      <div>
                        <div className="font-black text-slate-900 text-base">{opt.label}</div>
            ...
            <div className="text-center animate-in zoom-in duration-500">
              <div className="w-24 h-24 bg-emerald-100 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-5 shadow-inner">
                <CheckCircle className="w-12 h-12" />
              </div>
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-4 tracking-tighter">Ваша идеальная стратегия готова!</h2>
              <p className="text-slate-500 text-base mb-4 font-medium">Основываясь на ваших целях, мы рекомендуем тариф:</p>
              
              <div className="bg-white p-6 rounded-[2.5rem] border-2 border-emerald-500/20 shadow-2xl mb-4 relative overflow-hidden group">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button 
                  ...
                  className="px-10 py-5 bg-indigo-600 text-white rounded-2xl font-black text-base hover:bg-indigo-700 transition-all flex items-center justify-center gap-3 shadow-xl"
                >
                  Перейти к расчету <ArrowRight className="w-5 h-5" />
                </button>
                <button 
                  onClick={() => { setShowResult(false); setCurrentStep(0); setAnswers([]); }}
                  className="px-10 py-5 bg-slate-100 text-slate-500 rounded-2xl font-black text-base hover:bg-slate-200 transition-all flex items-center justify-center gap-3"
                >
