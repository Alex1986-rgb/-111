  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <header className="text-center mb-4">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-4 tracking-tighter leading-[1] max-w-5xl mx-auto">
          Масштабное создание <span className="gradient-text">SEO-контента</span> для тысяч страниц
        </h1>
        <p className="text-slate-500 max-w-3xl mx-auto text-base md:text-lg font-medium leading-relaxed">
      ...
      <div className="grid md:grid-cols-4 gap-4 mb-24">
         {[
           ...
           <section key={i} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
             <div className="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center mb-4 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                <benefit.icon className="w-7 h-7" />
             </div>
             <h3 className="text-base font-black text-slate-900 mb-3">{benefit.title}</h3>
             <p className="text-slate-500 text-xs leading-relaxed">{benefit.desc}</p>
           </section>
         ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start mb-24">
        <div className="lg:col-span-8 space-y-12">
          ...
          <section>
            ...
            <div 
              className={`relative group p-6 rounded-[3rem] border-4 border-dashed transition-all cursor-pointer text-center ${dragActive ? 'border-indigo-600 bg-indigo-50 scale-[1.01]' : 'border-slate-100 hover:border-indigo-200 bg-white shadow-sm'}`}
            ...
            >
              ...
              <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-[2.5rem] flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform">
                <Upload className="w-10 h-10" />
              </div>
              <h3 className="text-2xl font-black text-slate-900 mb-2">Загрузить файл со списком URL или ключевых слов</h3>
              <p className="text-slate-500 text-sm mb-4">Загрузите список страниц, которые нужно наполнить. Мы создадим для них контент, мета-теги и FAQ.</p>
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-4">
              {items.map((item, idx) => (
                <div key={item.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-end md:items-center group hover:border-indigo-200 transition-all">
          ...
          <section className="prose prose-slate max-w-none bg-slate-900 text-white p-6 rounded-[3.5rem] shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl"></div>
             <h2 className="text-3xl font-black text-white mb-4 tracking-tight">Как мы создаем десятки тысяч страниц?</h2>
             <div className="space-y-4 text-slate-300">
                ...
                <div className="mt-6 pt-12 border-t border-white/10">
                   <h3 className="text-lg font-black text-white mb-4">Пример структуры выгрузки (JSON/CSV/HTML):</h3>
  ...
  "faq": "#### Какие документы?..."
        ...
        <aside className="lg:col-span-4 sticky top-28">
          <div className="bg-white rounded-[3rem] p-6 text-slate-900 shadow-2xl border border-slate-100 relative overflow-hidden">
            <h3 className="text-2xl font-black mb-5 border-b border-slate-50 pb-6">Резюме проекта</h3>
            
            <div className="space-y-4 mb-4">
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm font-medium">Кол-во текстов:</span>
                <span className="text-lg font-black">{totalItems} шт.</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm font-medium">Общий объем:</span>
                <span className="text-lg font-black">{(totalSymbols / 1000).toFixed(1)}к знаков</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm font-medium">Базовая ставка:</span>
                <span className="text-lg font-black">{bulkService.pricePer1k} ₽/1к</span>
              </div>
            </div>

            <div className="p-6 bg-slate-900 rounded-[2.5rem] text-center mb-4 text-white shadow-xl shadow-slate-100">
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Общая стоимость</div>
              <div className="text-4xl font-black tracking-tight">{totalPrice.toLocaleString()} <span className="text-lg font-medium text-slate-500">₽</span></div>
            </div>

            <button 
              onClick={() => onConfirm(totalSymbols, totalItems, totalPrice)}
              className="w-full py-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2rem] font-black text-lg flex items-center justify-center gap-3 transition-all hover:scale-[1.02] shadow-xl shadow-indigo-600/20 active:scale-95"
            >
