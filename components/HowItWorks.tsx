    ...
    <section className="py-14 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-24">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-slate-50 text-slate-400 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 border border-slate-100">
            Workflow 2.0
          </div>
          <h2 className="text-3xl md:text-4xl font-black mb-4 tracking-tighter">Путь к <span className="text-indigo-600">ТОП-1</span> в 4 шага</h2>
          <p className="text-slate-500 font-medium text-base">Прозрачный процесс от первичного аудита до взрывного роста трафика.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 lg:gap-20 relative">
          <div className="hidden md:block absolute top-16 left-24 right-24 h-1 bg-slate-50 -z-0"></div>
          {steps.map((step, idx) => (
            <div key={idx} className="relative z-10 flex flex-col items-center text-center group">
              <div className="w-32 h-32 bg-white border-4 border-slate-50 rounded-[2.5rem] flex items-center justify-center mb-4 shadow-sm group-hover:border-indigo-600 transition-all duration-500">
